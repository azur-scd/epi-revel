...
function getAuthor($item){
    ...
    foreach ($auteursItem as $auteurItem): 
        ...
        if($auteurCount < sizeof($auteursItem)){$auteurStr = $auteurStr . ', ';} 
    endforeach; 
    return $auteurStr;
}
...
function buttonPdf($item){
    ...
    foreach($pdfId as $pdf){
        ...
        if(strpos($pdf, 'RXIV:')){
            $pdf = str_replace('ARXIV: ', '', $pdf);
            $pdfLink= 'http://arxiv.org/pdf/' . $pdf;
        }
    }
    return $pdfLink;
}
...
function index_auteurs($auteurs, $lettre){
    $output = '';
     foreach ($auteurs as $tempAuteur) {
        if (strtoupper($tempAuteur[0]) == $lettre){ 
    ...
    function index_mots($mots, $lettre){
        $output = '';
         foreach ($mots as $tempMot) {
            if (strtoupper($tempMot[0]) == $lettre){ 
                $output .= "<li>" ;
...
function type_traduction($type){
    ...
    if($type == 'publication-report'){
        $output = 'Rapport';
    }
    return $output;
}

function autre_traduction($mot){
    $output = $mot;
    if(get_html_lang() != 'fr'){
        $mot = strtolower($mot);
        if($mot == 'revues'){
...
function get_auteur_str($item){
    ...
    foreach ($auteursItem as $auteurItem){
        if($auteurCount < 4){
            $auteurStr = $auteurStr . clear_auteurs($auteurItem) ;
            $auteurCount = $auteurCount + 1;
            if(($auteurCount < $array_size) && ($auteurCount < 4)){$auteurStr = $auteurStr . ', ';}
        }
    }
    return $auteurStr;
}
