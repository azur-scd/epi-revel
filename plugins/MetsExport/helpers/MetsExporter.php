...
{ 
  ...
  private function _generateMetsBody($itemID,$recordType) {
    ...
    if(isset($item->Collection->id)){
    echo 'COL="DMD_COL_'.(metadata($item, 'Collection Name')). '" ';
    }
    echo ">\n";
  ...
  private function _generateMetsStructMap($itemID,$recordType="Item",$items=array()) {
    ...
    //création d'un tableau contenant les ID de toutes les sous collections
    $collectionOrder = get_db()->getTable('CollectionOrder_CollectionOrder');
    $subCollectionOrder = $collectionOrder->fetchOrderedCollections($itemID);


        //pour chaque ID de collection, on crée le METS correspondant
        foreach($subCollectionOrder as $current_col){
          $order_item = 0;
          $order_col = array_search($current_col, $subCollectionOrder) + 1;
          $recordCurrentCol = get_record_by_id('collection', $current_col['id']);
          echo '<METS:div ';
          echo 'TYPE="SOUS-COLLECTION" ';
          echo 'ID="COLLECTION_'. $current_col['id'] .'" ';
          echo 'LABEL="'. metadata($recordCurrentCol, array('Dublin Core', 'Title')) .'" ';
          echo 'ORDER="' . $order_col . '" ';
          echo ">\n";
          foreach($items as $item) {
            if(($item->Collection->id) == $current_col['id']) {
            $order_item = $order_item + 1;
            echo '<METS:div ';
            echo 'TYPE="ITEM" ';
            echo 'ID="ITEM_'.$item->id.'" ';
            echo 'LABEL="'.metadata($item, array('Dublin Core', 'Title')). '" ';
            echo 'ORDER="' . $order_item . '"';
            echo ">\n";
            
            //fin du mets de l'item
            echo "</METS:div>\n";
            
          }}

          //fin du mets de la sous collection
          echo "</METS:div>\n";
          }
