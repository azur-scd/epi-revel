...
class OaiPmhRepository_ResponseGenerator extends OaiPmhRepository_OaiXmlGeneratorAbstract
{
    ...
    private function listSets()
    {
        ...
        if(!$this->error) {
            ...
            foreach ($collections as $collection) {
                ...
                //suppression des actualités et des collections de revel dans la liste du dépot
                if($name != 'Actualités accueil' && strpos($name, 'href') == false){
                    $elements = array(
    ...
    private function listResponse($verb, $metadataPrefix, $cursor, $set, $from, $until) {
        ...
        //ajout d'un booleen pour verifier si on est dans un set ou non, car !$set provoque un problème
        $setBool = 0;
         
        if($set){
            $setId = get_record_by_id('collection', $set);
            $setOwner = $setId->getOwner();
            $allItems = $itemTable->fetchObjects($select); 
            $setBool = 1;
            foreach($allItems as $currentItem){
                $itemOwner = $currentItem->getOwner();
                if(metadata($itemOwner, 'name') == metadata($setOwner, 'name')){
                    $items[] = $currentItem;  
                }
            }
        } 
        ...
        if(count($items) == 0)
            ...
            elseif ($rows > ($cursor + $listLimit)) {
                ...
                $tokenElement->setAttribute('completeListSize', count($items)); //remplacer count($items) par $rows pour remettre comme avant
