...
class CleanUrl_View_Helper_GetRecordFullIdentifier extends Zend_View_Helper_Abstract
{
    ...
    protected function _getUrlPath($absolute, $withMainPath, $withBasePath)
    {
        ...
        elseif ($withBasePath) {
            $withMainPath = true;
            $AncestorTree = get_db()->getTable('CollectionTree')->getAncestorTree(metadata('collection', 'id'));
            $rootCollections = get_db()->getTable('CollectionTree')->getRootCollections();
            $AncestorTitle = '';
            if($AncestorTree) {
            foreach ($rootCollections as $rootCollection) {
                if(in_array($rootCollection, get_db()->getTable('CollectionTree')->getAncestorTree(metadata('collection', 'id')))){
        ...
        //remplacer $AncestorTitle et $maiPath pour revenir à l'url normale
